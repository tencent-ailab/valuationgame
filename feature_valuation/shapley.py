...
class ShapNN(object):
    ...
    def _build_train_op(self):
        ...
        learning_rate = tf.constant(self.learning_rate, tf.float32)
    ...
    def _final_layer(self, x, num_classes, mode):
        ...
        if mode == 'regression':
            ...
            regression_loss = tf.nn.l2_loss(self.predictions - self.labels)
    ...
    def _reg_loss(self, order=2):
        ...
        for var in tf.trainable_variables():
            if var.op.name.find(r'DW') > 0 or var.op.name.find(r'CW') > 0:
                if order == 2:
